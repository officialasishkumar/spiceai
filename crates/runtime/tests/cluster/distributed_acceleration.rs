    ...
    test_request_context()
        .scope(async {
            ...
            let plan_fmt = arrow::util::pretty::pretty_format_batches(&plan)
                .expect("format explain")
                .to_string();
            // The explain plan for distributed joins is non-deterministic — it depends
            // on how many executors have joined by the time the EXPLAIN runs (1 vs 2 executors
            // produce different Union/split structures). Validate key plan properties instead.
            assert!(
                plan_fmt.contains("HashJoinExec"),
                "Plan should use HashJoinExec for the join: {plan_fmt}"
            );
            assert!(
                plan_fmt.contains("FlightSqlExec"),
                "Plan should use FlightSqlExec for remote execution: {plan_fmt}"
            );
            assert!(
                plan_fmt.contains("SortPreservingMergeExec") || plan_fmt.contains("SortExec"),
                "Plan should include a sort for ORDER BY: {plan_fmt}"
            );
            assert!(
                plan_fmt.contains("Inner Join: t.id = c.id"),
                "Logical plan should show inner join on id: {plan_fmt}"
            );
